...
fn new_difficulty(difficulty: f64, grade: Grade) -> f64 {
    WEIGHTS[7] * initial_difficulty(Grade::Good) + (1f64 - WEIGHTS[7]) * (difficulty - WEIGHTS[6] * (grade as i32 - 3) as f64)
}
...
fn stability_after_recall(stability: f64, difficulty: f64, retrievability: f64, grade: Grade) -> f64 {
    ...
    stability * (WEIGHTS[8].exp() * (11.0 - difficulty) * stability.powf(-WEIGHTS[9]) * ((WEIGHTS[10] * (1.0 - retrievability)).exp() - 1.0) * factor + 1.0)
}
...
fn post_lapse_stability(stability: f64, difficulty: f64, retrievability: f64) -> f64 {
    WEIGHTS[11] * difficulty.powf(-WEIGHTS[12]) * ((stability + 1.0).powf(WEIGHTS[13]) - 1.0) * (WEIGHTS[14] * (1.0 - retrievability)).exp()
}
